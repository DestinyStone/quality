...
import org.springblade.modules.process_low.excel.ProcessLowExcel;
 ...
 * @Description:
 */
public class ProcessLowExcelUtil {
	private static final String EXCEL_NAME = "工序内不良台账.xlsx";

	public static ExcelWriter export(List<ProcessLowVO> lowVOList) {
		ExcelWriter writer = ExcelUtil.getWriter(true);
		setAlias(OutBuyQprExcel.class, writer);
		List<ProcessLowExcel> collect = lowVOList.stream().map(ProcessLowExcelUtil::convert).collect(Collectors.toList());
		writer.write(collect, true);
		return writer;
	}

	public static ExcelWriter export(List<ProcessLowVO> lowVOList, HttpServletResponse response) {
	...
	private static ProcessLowExcel convert(ProcessLowVO lowVO) {
		ProcessLowExcel copy = BeanUtil.copy(lowVO, ProcessLowExcel.class);
		copy.setType(typeMap.get(lowVO.getType()));
		copy.setLevel(levelMap.get(lowVO.getLevel()));
		copy.setApparatusType(apparatusMap.get(lowVO.getApparatusType()));
		copy.setTriggerAddress(triggerAddressMap.get(lowVO.getTriggerAddress()));
		...
		if (lowVO.getImgReportList() != null) {
			copy.setImgReportList(StringUtil.join(lowVO.getImgReportList().stream().map(BusFileVO::getUrl).collect(Collectors.toList())));
		}
